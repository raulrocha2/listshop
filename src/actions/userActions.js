...
import { 
    ...
    USER_PHONESALES_REQUEST,
    USER_PHONESALES_SUCCESS,
    USER_PHONESALES_FAIL,   
...
export const phoneSales = () => async (dispatch, getState) => {
    try {
        dispatch({
            type: USER_PHONESALES_REQUEST
        })
        ...
        const { data } = await axios.get(
            `/api/users/phone-sales/`,
            config
        )

        dispatch({
            type: USER_PHONESALES_SUCCESS,
            payload: data
        })
        localStorage.setItem('salesPhone', JSON.stringify(data))


    } catch (error) {
        dispatch({
            type: USER_PHONESALES_FAIL,
            payload: error.response && error.response.data.detail
                ? error.response.data.detail
                : error.message,
        })
